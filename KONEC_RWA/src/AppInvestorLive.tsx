...
export default function AppInvestorLive() {
  ...
  const checkBuyer = async () => {
    try {
      setBusy('load'); setError(''); const ph = await provider(); const buyer = ph.publicKey!; const salePk = currentSale(); const mintPk = currentPaymentMint(); const p = readProgram(); const opt: any = await p.account.paymentOption.fetch(optionPda(salePk, mintPk)); const treasuryAta = treasuryPaymentAta(salePk, mintPk); await ensureTokenAccount(treasuryAta, 'Treasury payment ATA');
