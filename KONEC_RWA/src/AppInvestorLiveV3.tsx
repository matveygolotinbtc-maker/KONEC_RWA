...
export default function AppInvestorLiveV3() {
  ...
  const [status, setStatus] = useState('Ready. Build buy-order-v3.');
  ...
  const load = async () => {
    try {
      ...
      const salePk = currentSale(); const mintPk = currentPaymentMint(); const p = readProgram(); const sale: any = await p.account.sale.fetch(salePk); const opt = optionPda(salePk, mintPk); const pay: any = await p.account.paymentOption.fetch(opt); const treasuryAuth = treasuryAuthority(salePk); const treasuryAta = treasuryPaymentAta(salePk, mintPk); await ensureTokenAccount(treasuryAta, 'Treasury payment ATA');
  ...
  const checkBuyer = async () => {
    try {
      setBusy('load'); setError(''); const ph = await provider(); const buyer = ph.publicKey!; const salePk = currentSale(); const mintPk = currentPaymentMint(); const p = readProgram(); const opt: any = await p.account.paymentOption.fetch(optionPda(salePk, mintPk)); const treasuryAuth = treasuryAuthority(salePk); const treasuryAta = treasuryPaymentAta(salePk, mintPk); await ensureTokenAccount(treasuryAta, 'Treasury payment ATA');
  ...
  const createBuyerAta = async () => {
    try {
      setBusy('ata'); setError(''); const ph = await provider(); const mintPk = currentPaymentMint(); const buyerAta = ata(ph.publicKey!, mintPk); if (!(await connection.getAccountInfo(buyerAta))) { const bh = await connection.getLatestBlockhash('confirmed'); const ix = new TransactionInstruction({ programId: ATA_PROGRAM_ID, keys: [{ pubkey: ph.publicKey!, isSigner: true, isWritable: true }, { pubkey: buyerAta, isSigner: false, isWritable: true }, { pubkey: ph.publicKey!, isSigner: false, isWritable: false }, { pubkey: mintPk, isSigner: false, isWritable: false }, { pubkey: SystemProgram.programId, isSigner: false, isWritable: false }, { pubkey: TOKEN_PROGRAM_ID, isSigner: false, isWritable: false }], data: Buffer.alloc(0) }); const tr = new Transaction({ feePayer: ph.publicKey!, recentBlockhash: bh.blockhash }).add(ix); const signed = await ph.signTransaction(tr); const sig = await connection.sendRawTransaction(signed.serialize()); await connection.confirmTransaction({ signature: sig, ...bh }, 'confirmed'); setTx(sig); }
